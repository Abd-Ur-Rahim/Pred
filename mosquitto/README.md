...
publish telemetry to Mosquitto, and the ingestion service subscribes to the
same topics before forwarding payloads to Kafka.
...
Mosquitto will be available at `tcp://localhost:1883`.
...
`MQTT_BROKER=tcp://mosquitto:1883` so it connects over the Compose network.
...
- [ ] Use TLS for broker access outside a trusted development network
